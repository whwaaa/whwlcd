...
void jl188a_lcd_clear( void ) {
    for ( uint8_t page=0; page<8; page++ ) {
        //设置页地址
        jl188a_page_address_set(page);
        //设置列地址
        jl188a_column_address_set(0);
        //写入数据
        for ( int i=0; i<132; i++ ) {
            jl188a_display_data_write(0x00);
        }
    }
}
...
void jl188a_lcd_init( void ) {
    HAL_GPIO_WritePin(Jl188a_RST_GPIO_Port, Jl188a_RST_Pin, GPIO_PIN_SET);
    HAL_Delay(100);
    HAL_GPIO_WritePin(Jl188a_RST_GPIO_Port, Jl188a_RST_Pin, GPIO_PIN_RESET);
    HAL_Delay(100);
    HAL_GPIO_WritePin(Jl188a_RST_GPIO_Port, Jl188a_RST_Pin, GPIO_PIN_SET);
    HAL_Delay(200);
    ...
    jl188a_reset();//复位
    HAL_Delay(100);
    jl188a_power_controller_set(4);//Voltage Coboost ON 
    HAL_Delay(10);
    jl188a_power_controller_set(6);//Voltage Regulator ON 
    HAL_Delay(10);
    jl188a_power_controller_set(7);//Voltage Follower ON 
    HAL_Delay(10);
    //1.效果最佳
    //jl188a_lcd_bias_set(1);
    //jl188a_v5_voltage_regulator_internal_resistor_ratio_set(2);
    //2.次之
    //jl188a_lcd_bias_set(0);
    //jl188a_v5_voltage_regulator_internal_resistor_ratio_set(4);
    jl188a_lcd_bias_set(1);//默认设置Bias 1/9
    HAL_Delay(10);
    jl188a_v5_voltage_regulator_internal_resistor_ratio_set(2);//电阻率选择
    HAL_Delay(10);
    jl188a_the_electronic_volume(0);//对比度设置
    HAL_Delay(10);
    jl188a_display_start_line_set(0);//初始化显示行0-63
    HAL_Delay(10);
    //jl188a_display_all_points_ON_OFF(1);
    jl188a_common_output_mode_select(1);//正方向
    jl188a_display_on_off(1);//开启显示
    // jl188a_display_normal_reverse(1);//反显
    jl188a_lcd_clear();
}
...
void jl188a_writeDataToRAM( uint8_t page, uint8_t startX, uint8_t endX, uint8_t *pdata ) {
    ...
    jl188a_page_address_set(page);
    //设置列地址
    jl188a_column_address_set(startX);
    //写入数据
    for ( uint8_t i=0; i<xlen; i++ ) {
        jl188a_writeByteData(pdata[i]);
    }
}
                ...
                for ( uint8_t idx=0; idx<(sizeof(asciiFontIdx)); idx++ ) {
                     if ( data[b] == asciiFontIdx[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<8&&xlen_t>0; j++ ) {
                            xlen_t--;
                            jl188a_writeByteData(asciiFont_t[idx][i*8+j]);
                        }
                        break;
                    }
                }
                ...
                for ( uint8_t idx=0; idx<(sizeof(asciiFontIdx)); idx++ ) {
                     if ( data[b] == asciiFontIdx[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<8&&xlen_t>0; j++ ) {
                            xlen_t--;
                            if ( i == 0 ) {
                                jl188a_writeByteData(asciiFont_t[idx][i*8+j]<<yu);
                            } else if ( i == 1 ) {
                ...
                for ( uint8_t idx=0; idx<(sizeof(myFontIdx_16)/3); idx++ ) {
                    if ( (uint32_t)(font[b*3]<<16|font[b*3+1]<<8|font[b*3+2]) == myFontIdx_16[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<16&&xlen_t>0; j++ ) {
                            xlen_t--;
                            jl188a_writeByteData(myFont_16[idx*2+i][j]);
                        }
                        break;
                    }
                }
                ...
                for ( uint8_t idx=0; idx<(sizeof(myFontIdx_16)/3); idx++ ) {
                    if ( (uint32_t)(font[b*3]<<16|font[b*3+1]<<8|font[b*3+2]) == myFontIdx_16[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<16&&xlen_t>0; j++ ) {
                            xlen_t--;
                            if ( i == 0 ) {
                                jl188a_writeByteData(myFont_16[idx*2+i][j]<<yu);
                            } else if ( i == 1 ){
                                jl188a_writeByteData(myFont_16[idx*2+i-1][j]>>(8-yu)|(myFont_16[idx*2+i][j]<<yu));
                            } else if ( i == 2 ) {
                                jl188a_writeByteData(myFont_16[idx*2+i-1][j]>>(8-yu));
                            }
                        }
                        break;
                    }
                ...
                for ( uint8_t idx=0; idx<(sizeof(myFontIdx_24)/3); idx++ ) {
                    if ( (uint32_t)(font[b*3]<<16|font[b*3+1]<<8|font[b*3+2]) == myFontIdx_24[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<24&&xlen_t>0; j++ ) {
                            xlen_t--;
                            jl188a_writeByteData(myFont_24[idx*3+i][j]);
                        }
                        break;
                    }
                }
                ...
                for ( uint8_t idx=0; idx<(sizeof(myFontIdx_24)/3); idx++ ) {
                    if ( (uint32_t)(font[b*3]<<16|font[b*3+1]<<8|font[b*3+2]) == myFontIdx_24[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<24&&xlen_t>0; j++ ) {
                            xlen_t--;
                            if ( i == 0 ) {
                                jl188a_writeByteData(myFont_24[idx*3+i][j]<<yu);
                            } else if ( i == 1 ) {
                                jl188a_writeByteData((myFont_24[idx*3+(i-1)][j]>>(8-yu))|(myFont_24[idx*3+i][j]<<yu));
                            } else if ( i == 2 ) {
                                jl188a_writeByteData((myFont_24[idx*3+(i-1)][j]>>(8-yu))|(myFont_24[idx*3+i][j]<<yu));
                            } else if ( i == 3 ) {
                                jl188a_writeByteData((myFont_24[idx*3+(i-1)][j]>>(8-yu)));
                            }
                        }
                        break;
                    }
                ...
                for ( uint8_t idx=0; idx<(sizeof(myFontIdx_32)/3); idx++ ) {
                    if ( (uint32_t)(font[b*3]<<16|font[b*3+1]<<8|font[b*3+2]) == myFontIdx_32[idx] ) {//查找到font索引
                        ...
                        for ( uint8_t j=0; j<32&&xlen_t>0; j++ ) {
                            xlen_t--;
                            if ( i == 0 ) {
                                jl188a_writeByteData(myFont_32[idx*4+i][j]<<yu);
                            } else if ( i == 1 ) {
                                jl188a_writeByteData((myFont_32[idx*4+(i-1)][j]>>(8-yu))|(myFont_32[idx*4+i][j]<<yu));
                            } else if ( i == 2 ) {
                                jl188a_writeByteData((myFont_32[idx*4+(i-1)][j]>>(8-yu))|(myFont_32[idx*4+i][j]<<yu));
                            } else if ( i == 3 ) {
                                jl188a_writeByteData((myFont_32[idx*4+(i-1)][j]>>(8-yu))|(myFont_32[idx*4+i][j]<<yu));
                            } else if ( i == 4 ) {
                                jl188a_writeByteData((myFont_32[idx*4+(i-1)][j]>>(8-yu)));
                            }
                        }
                        break;
                    }
...
void jl188a_writeLogo_0( uint8_t x, uint8_t y ) {
    ...
    for ( uint8_t i=0; i<4&&page<6; i++,page++ ) {
        ...
        if ( yu == 0 ) {
            for ( uint8_t j=0; j<32&&xlen_t>0; j++ ) {
                xlen_t--;
                jl188a_writeByteData(logo_0[i][j]);
            }
        } else {
            for ( uint8_t j=0; j<32&&xlen_t>0; j++ ) {
                xlen_t--;
                if ( i == 0 ) {
                    jl188a_writeByteData(logo_0[i][j]<<yu);
                } else {
                    jl188a_writeByteData(logo_0[i-1][j]>>(8-yu)|logo_0[i][j]<<yu);
                }
            } 
        }
    }
